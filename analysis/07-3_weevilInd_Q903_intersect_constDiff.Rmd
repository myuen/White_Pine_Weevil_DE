...
sidf.wide <- read.table("../results/limma-results-focus-terms.wide.15July.tsv")
row.names(sidf.wide) <- 
  as.character(gsub("WPW_Inoculation_Trinity_BBMerged_31May2016_TRINITY_", "", row.names(sidf.wide)))
...
intersection_row_dist <- as.dist(1 - cor(t(intersection_avgCPM), method = "spearman"))
intersection_row_hclust <- hclust(intersection_row_dist, "complete")
intersection_row_dendrogram <- as.dendrogram(intersection_row_hclust)

intersection_col_dist <- as.dist(1 - cor(intersection_avgCPM, method = "spearman"))
intersection_col_hclust <- hclust(intersection_col_dist, "complete")
